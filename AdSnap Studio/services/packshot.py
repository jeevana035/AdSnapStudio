...
def create_packshot(
    ...
    Args:
        api key: Bria AI API key
        inage_data: Image data in bytes
        background_color: Background color in hex format or 'transparent'
        sku: Optional SKU identifier for the product
        force_rmbg: Whether to force background removal even if alpha channel ex 
        content_moderations: Whether to enable content moderation
    ...
    try:
        ...
        print(f"Response statuc: {response.status_code}")
