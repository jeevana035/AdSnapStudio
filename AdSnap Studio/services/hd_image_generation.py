...
def generate_hd_image(
    ...
    Args:
        ...
        num results: Number of images to generate (1-4)
        aspect_ratio: Image aspect ratio ("1:1", "2:3", "3:2", etc.)
        sync: Whether to wait for results or get URLs immediately
        seed: Optional see for reproducible results
        negative_prompt: ments to exclude from generation
        ...
        content moderation: Whether to enable content moderation
    ...
    try:
       ...
       raise Exception(f"HD image generatu=ion failed: {str(e)}")  